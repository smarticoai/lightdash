...
export class ProjectService extends BaseService {
    ...
    */
    ...
    async _getWarehouseClient(
        ...
        const cacheKey = `${projectUuid}${snowflakeVirtualWarehouse || ''}${databricksCompute || ''
            }`;
    ...
    async compileQuery(
        ...
        const explore =
            'explore' in args
                ? args.explore
                : await this.getExplore(account, projectUuid, args.exploreName, undefined, undefined);
    ...
    private async runQueryAndFormatRows({
        ...
        return wrapSentryTransaction(
            ...
            async (span) => {
                ...
                this.logger.info(
                    `Query returned ${rows.length} rows and ${Object.keys(rows?.[0] || {}).length
                ...
                this.logger.info(
                    `Formatted rows returned ${formattedRows.length} rows and ${Object.keys(formattedRows?.[0] || {}).length
                    } columns`,
                );
                return {
                    rows: formattedRows,
                ...
                const warehouseResults = await wrapSentryTransaction(
                    ...
                    async () => {
                        try {
                            ...
                            this.logger.warn(
                                `Error running "${warehouseClient.credentials.type
                                }" warehouse query:
                                "${query}"
                                with query tags:
            ...
            async (span) => {
                try {
                    ...
                    if (explore.tables && userAttributes?.bq_project_id) {
                        const bqProjectId: string = (userAttributes?.bq_project_id as unknown as string[])[0];
    ...
    async searchFieldUniqueValues(
        ...
        // SMR-START
        if (explore && explore.tables && userAttributes?.bq_project_id) {
            const bqProjectId: string = (userAttributes.bq_project_id as unknown as string[])[0];
            for (const [, compiledTable] of Object.entries(explore.tables)) {
                compiledTable.database = bqProjectId;
                compiledTable.sqlTable = compiledTable.sqlTable.replace(process.env.SMR_BQ_PROJECT || 'project-not-defined', bqProjectId);
    ...
    */
    ...
    async findReplaceableCustomFields({
        ...
        this.logger.info(
            `Found ${Object.keys(replaceableFields).length
        ...
        const updatedChartPromises = Object.entries(replaceFields).map(
            async ([chartUuid, fieldsToReplace]) => {
                ...
                if (Object.keys(skippedFields.customMetrics).length > 0) {
                    ...
                    this.logger.info(
                        `Skipped replace custom fields in chart ${chart.uuid
                        }:\n ${skippedReasons.join('\n')}`,
                    );
                }
